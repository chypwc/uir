...
A finite interaction record can end because the modelled task has ended or because an external collector has stopped recording. These cases may produce trajectory files of the same length, but they make different claims about what could happen next. The distinction is useful because later return and value calculations must know whether future task rewards truly cease or merely become unobserved.
...
Let a task-defined finite horizon be $N\in\mathbb N$ decisions, with actions selected at times $t\in\{0,\ldots,N-1\}$. The transition to $S_N$ completes the task because the horizon is part of its definition. This is termination, not truncation. If the transition and decision laws can depend on the number of decisions remaining, a stationary Markov representation must include that information. Define the augmented state space and state by
...
The second component records the number of decisions remaining. The augmented terminal set is
...
5. **Validity condition:** A task lasts exactly $N$ decisions, but the optimal action can depend on the number of decisions remaining. What must a stationary Markov state representation contain?

### Answers and hints {.unnumbered}

1. A state-based transition terminates when its realised next state satisfies $S_{t+1}\in\mathcal S_{\mathrm{term}}$. A task-defined finite horizon is also termination and can be expressed by augmenting the state with the remaining decision count.
...
5. The state must include the relevant stage information, such as the remaining count $N-t$. Otherwise identical physical-state labels can require different transition or decision laws at different stages.

## References {.unnumbered}

- Richard S. Sutton and Andrew G. Barto, *Reinforcement Learning: An Introduction*, second-edition draft, Chapter 3, especially Sections 3.1--3.4, 3.6, and 3.7 on the agent--environment interaction, rewards, episodic and continuing tasks, terminal-state extensions, finite-MDP dynamics, policies, and policy-dependent state evolution.
- Dimitri P. Bertsekas, *Reinforcement Learning and Optimal Control*, second edition, Chapter 1, especially Sections 1.3.1, 1.6.2, and 1.6.4 on stochastic controlled models, termination states, and finite-to-infinite-horizon reformulations.
- Zhiqing Xiao, *Reinforcement Learning: Theory and Python Implementation*, Chapters 1 and 2, especially the agent--environment interface and the finite-MDP treatment of terminal states and trajectories.
- Farama Foundation, *Gymnasium Documentation*, “Handling Time Limits,” on the distinction between task-defined termination and externally imposed truncation.
