...
- **Active task:** Review the drafted RViz implementation example in Chapter 11, including the attached static PNG and HTML animation.
...
| `P1.3` — General kinematics and Jacobians | Complete Chapter 10 and its Chapter 11 implementation companion | Extend `rigid_body_kinematics` with bounded forward and velocity kinematics; add a scenario-specific downstream `rigid_body_kinematics_visualization` consumer with ROS 2, `visualization_msgs`, and RViz | Active: geometry/rejection tests pass; review the drafted RViz companion example |
...
- [x] Complete the agreed space-form forward-kinematics implementation and focused test batch: [15 evaluator tests](ros_ws/src/rigid_body_kinematics/test/test_forward_kinematics.cpp), plus [limit](ros_ws/src/rigid_body_kinematics/test/test_joint_limits.cpp), [joint](ros_ws/src/rigid_body_kinematics/test/test_joint_definition.cpp), and [model](ros_ws/src/rigid_body_kinematics/test/test_serial_chain_model.cpp) construction tests. Closed at the user's requested scope on 2026-09-07; this does not close the complete cycle or claim full specification coverage.
  - Verification: from the sourced and activated `ros_ws`, `cmake --build build/rigid_body_kinematics --target test_joint_limits test_joint_definition test_serial_chain_model test_forward_kinematics -j2` succeeded; `ctest --test-dir build/rigid_body_kinematics -R '^test_(joint_limits|joint_definition|serial_chain_model|forward_kinematics)$' --output-on-failure` passed all four suites, comprising 30 tests with no failures or skips.
  - Acceptance exceptions remain explicit against the [specification](docs/02_spatial_kinematics/general_kinematics_and_jacobians.md): the user skipped reuse/default-policy equivalence in GK-ACC-012; its interval-boundary portion is tested. The explicit reversed-factor negative control in GK-ACC-008 is not implemented. The additional joint-product-overflow test was not selected; screw-scaling and final home-pose overflow are tested. Reconcile the specification acceptance exceptions before full-cycle closure; no requirement is silently waived by closing this test batch.
- [ ] Finish review of the [spatial 2R visualization specification](docs/02_spatial_kinematics/spatial_2r_forward_kinematics_visualization.md), including animation bounds, failure behaviour, and headless acceptance checks. The [worked geometric example](notes/part_01_motion_mechanics_control/11_general_robot_kinematics_and_jacobians_implementation.qmd#sec-general-kinematics-impl-spatial-2r), perpendicular home axes, two models, and static markers have been discussed and used in implementation; full specification approval is not yet recorded.
- [x] Create [rigid_body_kinematics_visualization](ros_ws/src/rigid_body_kinematics_visualization) as a separate C++20 `ament_cmake` package with one scenario-specific `spatial_2r_forward_kinematics_demo` executable, directly declared ROS message dependencies, and installed-core linkage; no reusable public visualization library.
- [x] Implement and visually inspect the static configuration at $(q_1,q_2)=(\pi/2,-\pi/2)$: core-evaluated elbow and end-effector poses, joint spheres, connecting links, and three RGB end-effector axis arrows in `world`.
  - Progress evidence (2026-09-08): the user built and ran the [demo](ros_ws/src/rigid_body_kinematics_visualization/src/spatial_2r_forward_kinematics_demo.cpp), reported `Prepared 5 marker messages.`, and supplied an RViz screenshot showing all five markers with `MarkerArray` status `Ok`. The missing two-point arrow assignment was corrected, and RViz's fixed frame was changed from `map` to `world`. This is manual display evidence, not headless numerical acceptance; the subsequently saved PNG is linked below.
- [x] Add and visually inspect the pose-text marker (ID 5) and user-requested end-effector `x`, `y`, `z` text labels (IDs 7–9); retain ID 6 for the trail. The static display now contains nine markers, with the z arrow and label brightened for readability.
- [x] Extract `make_spatial_2r_markers(elbow_model, end_model, u, stamp)` to validate the bounded input, evaluate both poses, and construct a complete timestamped marker array. The user reported the static extraction working before animation was added.
- [x] Implement the eight-second triangle-wave animation using elapsed steady-clock time, with startup `animate=false` retaining the static pose and `animate=true` varying $\mathbf q(u)=u[\pi/2\;-\pi/2]^{\mathsf T}\,\mathrm{rad}$. The timer calls the shared helper for fresh poses, arrows, and labels; both models still use the installed core, with no duplicated forward-kinematics equations. The user reports a working animation and supplied a GIF; automated animation/marker acceptance is not yet recorded.
- [x] Save the [static pose PNG](notes/figures/spatial_2r_space_poe_rviz.png) and [draft animation GIF](notes/figures/spatial_2r_space_pose_rivz.gif). The PNG was visually inspected and contains a readable, cropped RViz scene. The GIF is 10,972,711 bytes (about 10.46 MiB), exceeding the 5 MiB target; its duration, looping, and book-width readability remain to be checked. Preserve the supplied files; reconcile the GIF's current filename with the planned `spatial_2r_space_poe_rviz.gif` when preparing final media.
- [ ] Reconcile the visualization specification with the requested axis labels and lighter z colour: reserve IDs 7–9 for labels and increase the maximum marker count from seven to ten including the future trail; record the reviewed scope before final acceptance.
- [x] Add the [home/nonzero geometry and invalid-input tests](ros_ws/src/rigid_body_kinematics_visualization/test/test_spatial_2r_markers.cpp) for the same marker-building implementation compiled into the demo. Rebuilt the package and reran the registered `test_spatial_2r_markers` target on 2026-09-08 after moving the header under `include/rigid_body_kinematics_visualization`: both GoogleTests passed with no failures or skips.
- [ ] **Active:** Review the drafted [RViz implementation example](notes/part_01_motion_mechanics_control/11_general_robot_kinematics_and_jacobians_implementation.qmd#sec-general-kinematics-impl-rviz), its essential excerpts, media, and the extension to the existing cycle-wide connection summary. The chapter HTML rendered successfully; an isolated PDF layout preview of the new block and diagram was rendered and inspected. The supplied eight-second looping GIF is attached unchanged by user request; its 10.46 MiB size still exceeds the final-media target.
...
- [ ] Complete the remaining bounded-history/stable-ID check when the trail is implemented. The geometry and invalid-input tests above cover the existing helper; live RViz inspection remains explanatory evidence rather than a numerical acceptance oracle.
- [ ] After the visualization implementation and focused tests work, extend `notes/part_01_motion_mechanics_control/11_general_robot_kinematics_and_jacobians_implementation.qmd` retrospectively with the visualization launch command, core-to-marker mapping, and visual-evidence boundary; complete review of the core representation, equation-to-code mapping, multiplication order, public interface, validation, and test meaning. Always retain `notes/figures/spatial_2r_space_poe_rviz.png`; attempt a 6–8 second `notes/figures/spatial_2r_space_poe_rviz.gif` for HTML and retain it only when it is legible at book width, loops cleanly, contains no unrelated desktop UI, and is no larger than 5 MiB; use the PNG for PDF and as the HTML fallback.
