...
All phases belong to the learning programme. Phase 7 is the synthetic intelligence-engine integration capstone; Phase 10 is the required Generalization and Adaptation Benchmark; Phases 11 and 14 are distinct required release-grade capstone products. Epistemic autonomy is a separate optional future capstone and does not block this table.
...
| `M2` — Reusable learning foundation             |      6 | Robotics acceptance of installed reinforcement-learning, data, checkpoint, Transformer, diffusion, safety-cost, and evaluation cores through mobile and non-planar consumers                                  | Queued   | —                                                                                                            |
| `M3` — Synthetic Intelligence Engine Capstone   |      7 | Controlled-text integration of a tiny autoregressive Transformer, memory, grounded latent state, world-model imagination, and learned reasoning control                                                         | Queued   | —                                                                                                            |
...
| `M6` — Generalization and Adaptation Layer      |     10 | Two-source/one-held-out-arm benchmark with reusable robot, environment, task, adaptation, task-action, data, and evaluation contracts                                                                             | Queued   | —                                                                                                            |
| `M7` — Capstone Product A                       |     11 | Core Intelligence Release                                                                                                                                                                                          | Queued   | —                                                                                                            |
| `M8` — Classical UAV platform                   |     12 | Frozen classical UAV inspection-and-return baseline                                                                                                                                                                | Queued   | —                                                                                                            |
| `M9` — UAV intelligence transfer                |     13 | Learned prediction, planning, adaptation, memory, uncertainty, and safety transfer to flight                                                                                                                       | Queued   | —                                                                                                            |
| `M10` — Capstone Product B                      |     14 | Multi-embodiment Intelligence Transfer Release                                                                                                                                                                     | Queued   | —                                                                                                            |

## Parallel intelligence track

The parallel track is active at `IF.4` — Finite MDPs, Bellman equations, and exact dynamic programming. Former `IF.1`–`IF.3` surveys are retired; their mathematical topics are written only as just-in-time prerequisite blocks when an active consumer first needs them. The active task, ordered capability cycles, phase destinations, and evidence belong only in [INTELLIGENCE_CHECKLIST.md](INTELLIGENCE_CHECKLIST.md). When this main frontier reaches an early-completed intelligence artifact, review and extend that artifact in place; do not create a duplicate implementation or note.
...
- [ ] After the implementation and focused tests work, create the first substantive section of `notes/11_general_robot_kinematics_and_jacobians_implementation.qmd` and review its serial-chain representation, equation-to-code mapping, multiplication order, public interface, validation, test meaning, visualization launch command, core-to-marker mapping, and visual-evidence boundary. Always retain `notes/figures/planar_2r_space_poe_rviz.png`; attempt a 6–8 second `notes/figures/planar_2r_space_poe_rviz.gif` for HTML and retain it only when it is legible at book width, loops cleanly, contains no unrelated desktop UI, and is no larger than 5 MiB; use the PNG for PDF and as the HTML fallback.
...
### P1.4 — General inverse-kinematics foundations and planar manipulator laboratory

The learning outcome is a reusable inverse-kinematics foundation for general $n$-joint open serial chains and declared Euclidean or spatial-pose tasks. Its executable evidence remains the first stage of one educational planar-manipulator side project: inspectable 2R/3R analytic and numerical experiments that exercise the general ideas without starting the Phase 8 spatial manipulation product. Phase 1 does not implement collision-aware, contact-aware, or globally complete constrained inverse kinematics.

#### Learn and review

- [ ] Draft `notes/12_inverse_kinematics_redundancy_and_planar_manipulators.qmd` from *Modern Robotics*, Chapter 6, while reusing Chapters 4–5, supported by Craig, Chapters 3–5, and Corke, Chapters 7–8; teach the general definitions and local numerical methods before using planar 2R/3R arms as the concrete analytic and implementation examples.
- [ ] Define inverse kinematics for a general $n$-joint open serial chain from a declared task map $\mathbf f:\mathcal Q\rightarrow\mathcal Y$ or pose map $\mathbf F_e:\mathcal Q\rightarrow SE(3)$; distinguish existence, reachability, uniqueness, branch choice, exact solutions, approximate solutions, local convergence, and solver failure.
- [ ] For a desired spatial pose $\mathbf T_{sd}$, define body- and space-frame pose errors from $\mathbf T_{se}(\mathbf q)^{-1}\mathbf T_{sd}$ and $\mathbf T_{sd}\mathbf T_{se}(\mathbf q)^{-1}$ using the reviewed principal $SE(3)$ logarithm; connect each error to the matching body or space Jacobian and keep translational and angular stopping tolerances separate.
...
- [ ] Add a deterministic offline scenario runner that writes a versioned machine-readable result table; use a small Python plotting script only for residual, iteration, branch, joint-limit, and metric histories that RViz does not communicate clearly.
- [ ] Extend the downstream `rigid_body_kinematics_visualization` package to replay selected versioned 2R/3R configurations, targets, and solution branches from the accepted scenario results without adding ROS dependencies to `planar_manipulator_lab`; extract shared marker helpers only when this second consumer demonstrates common behaviour with the P1.3 scenario.
...
- [ ] Link the approved theory, specification, companion, packages, deterministic tests, scenario manifest, RViz assets, and numerical-history plots and close the kinematics stage of `planar_manipulator_lab`.
...
- [ ] Continue `notes/part_01_motion_mechanics_control/16_dynamic_systems_mechanics_and_actuator_limits.qmd`, preserving all useful material from the renamed draft.
...
- [ ] Consult current official ROS 2, RViz, and Gazebo documentation for simulation time, joint state, the selected wheel-effort boundary, launch, bridging, headless execution, and visualization of declared paths and frames; do not rely on stale textbook software guidance.
...
- [ ] When implementation begins, create `notes/26_ros2_gazebo_integration_and_motion_safety_implementation.qmd` with the durable simulation-clock, launch, bridge, effort-adapter, frame, timestamp, evaluator, Gazebo-physics, and RViz-visualization boundaries, including one representative RViz figure under the same PNG-first media rule used by P1.3.
- [ ] Create `ros_ws/src/differential_drive_gazebo` with the minimum robot, world, headless launch, optional GUI launch, saved RViz configuration, nominal scenario runner, and evaluator required by the specification.
...
- [ ] Display the declared reference path, odometry path, and frames in RViz while Gazebo alone owns physical motion; do not route RViz data or evaluator-only ground truth back into the controller stack.
...
- [ ] Launch and inspect the optional Gazebo-plus-RViz view for the nominal scenarios, preserve the representative note figure, and treat this GUI inspection as supplementary to the unchanged headless acceptance path.
...
- [ ] Link the integration specification, implementation companion, packages, headless launch tests, optional visualization assets, and nominal results and close `P1.11`.
