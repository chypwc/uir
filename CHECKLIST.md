...
- **Active stage:** Learn and write.
- **Active task:** Review the first [space-Jacobian learning block](notes/part_01_motion_mechanics_control/10_general_robot_kinematics_and_jacobians.qmd#sec-end-effector-pose-rate-space-twist) and its retrieval questions: extracting the space twist from the pose derivative and distinguishing its linear block from the end-effector origin velocity. The joint-rate mapping and ordered-product column derivation remain to be written.
- **Next engineering checkpoint:** Review and quiz the required space-Jacobian theory, then extend its specification before implementation.
...
- [ ] **Active:** Learn and write spatial end-effector velocity, the derivation of the space Jacobian from the pose map, and the configuration-dependent transformation of its columns using *Modern Robotics*, Chapter 5, Craig, Chapter 5, and Corke, Chapter 8. The first [pose-rate and space-twist block](notes/part_01_motion_mechanics_control/10_general_robot_kinematics_and_jacobians.qmd#sec-end-effector-pose-rate-space-twist) is drafted for learner review, with a rotating-link example and retrieval questions. It recalls Chapter 7 rather than repeating its full derivation. The Jacobian mapping and configuration-dependent columns are not yet drafted; the remaining headings are placeholders, not completed learning content.
