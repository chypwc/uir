...
- **Active task:** Review the spatial 2R forward-kinematics visualization specification and its worked geometric example before creating the consumer package.
...
- [ ] **Active:** Review the [spatial 2R visualization specification](docs/02_spatial_kinematics/spatial_2r_forward_kinematics_visualization.md) and [worked geometric example](notes/part_01_motion_mechanics_control/11_general_robot_kinematics_and_jacobians_implementation.qmd#sec-general-kinematics-impl-spatial-2r), including the perpendicular home axes, two models, static-first delivery, marker contract, animation bounds, failure behaviour, and headless acceptance checks.
- [ ] Create `ros_ws/src/rigid_body_kinematics_visualization` as a separate C++ `ament_cmake` package with one scenario-specific `spatial_2r_forward_kinematics_demo` executable that depends on `rclcpp`, `visualization_msgs`, and the installed `rigid_body_kinematics` public interface; do not add a reusable public visualization library in this cycle.
- [ ] Animate the specified spatial 2R scenario with a triangle-wave scalar $u$ that travels from $0$ to $1$ and back, using $\mathbf q(u)=u[\pi/2\;-\pi/2]^{\mathsf T}\,\mathrm{rad}$ and home axes $+z_s$ at the base and $+y_s$ at the elbow. Obtain elbow and end-effector positions by evaluating one-joint and two-joint `SerialChainModel` fixtures through the core rather than duplicating forward-kinematics equations or adding intermediate-link output to the core API; the base position is fixed. Retain GK-ACC-007 as a separate planar core test, not the visualization scenario.
...
- [ ] Add the specified essential headless visualization checks: one home/nonzero geometry test for core-to-marker agreement and independent spatial 2R joint positions and axis endpoints, one bounded-history/stable-ID check, and one invalid-input rejection check. Treat the live RViz inspection as explanatory evidence rather than a numerical acceptance oracle.
