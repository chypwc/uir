...
- From `IF.4` onward, implement one mathematical operation or algorithmic cycle at a time in modern C++20. Use LibTorch dense tensors for the finite-MDP compute representation, policy evaluation, policy/value iteration, and tabular Q-learning, then reuse LibTorch for neural components. Keep Python limited to independent numerical references, plotting, dataset generation, and experiment orchestration.
...
- Keep validated semantic models separate from numerical compute views. Retain the finite-MDP outcome records and Kahan validation; construct a separate owning dense tensor representation for repeated computations rather than replacing the model's internal storage. Vectorise independent arithmetic and batch independent problems or learners. Keep CPU and CUDA execution in the same LibTorch implementation, with CPU fallback when CUDA is unavailable; verify numerical agreement within declared tolerances and measure end-to-end cost before selecting GPU execution as the performance default. Scalar loops remain appropriate for validation, assembly, independent reference calculations, and mathematically sequential updates.
...
- **Active task:** Resolve the expected-reward numerical-domain/failure policy in the [finite-MDP specification](docs/05_optimisation_planning_decisions/finite_mdp_model.md), using its [overflow reproducer](docs/05_optimisation_planning_decisions/finite_mdp_model.md#unresolved-expected-reward-numerical-domain). Specify the chosen behaviour before changing production code; this is the next contract refinement within the still-open implementation stage.
...
- **Blockers:** Full-domain expected-reward acceptance remains blocked by the unresolved overflow policy. Cycle 1 also retains dense tensor conversion, policy-induced calculations, fixture work, and final verification; the semantic-model review does not close the cycle.
...
| `IF.13`–`IF.14` — Synthetic intelligence engine | Phase 7, with later Phase 11 reuse | Permanent synthetic reference consumer and linked integration evidence | Accept the cores into the main programme; retain robot-specific models and adapters downstream |
...
|---|---|---|
| `IF.5` sampled tabular learning | Relevant `IF.4` model and exact-value cycles; accepted LibTorch tensor foundation for dense Q tables | Neural models and continuous fixtures |
...
The first tensor consumer owns the shared LibTorch build and CPU/CUDA tensor foundation, normally the `IF.4` dense compute work unless another consumer arrives first. The first neural consumer extends that same toolchain evidence with automatic differentiation, optimiser updates, and checkpoint checks; non-neural table computations do not wait for those neural prerequisites. The first continuous consumer owns the point-mass fixture cycle, whether it arrives through PPO, TD3/SAC, cloning, or learned dynamics. Shared prerequisites are not locked behind completion of their default capability number.
...
- Keep theory and retrieval tests in the main book, requirements under `docs/`, project-owned production cores and deterministic tests under `ros_ws/src`, and detailed results and manifests with their owning artifacts. Record contract findings in the owning specification and concise capstone or merge conclusions here, with links to existing evidence; no separate report is required.
...
- Additional 2026-09-05 review evidence: the three GoogleTest executables contained 45 unit cases (2 index, 12 distribution, 31 finite-MDP); formatting passed, but the 11 cppcheck skips were not executed evidence. An independent binary64 probe of `1, 2^-53, 2^-53` returned ordinary sum `1` and compensated sum `1.0000000000000002`, with corrections `0, -2^-53, 0`. A production distribution with probabilities `0.5, 2^-54, 2^-54, 0.5 - 2^-53`, distinct rewards at one destination, and tolerance `1e-18` was accepted and returned marginal `1`; these were manual diagnostics, not new tests or full-domain guarantees. Quarto 1.9.38 HTML and book-format PDF assembly (`--no-execute`) passed with the repository environment active; Kahan equations, trace table, code layout, and cross-references were inspected. Temporary outputs were removed. This did not verify policy application, dense conversion, or complete fixture work.
- [ ] Extend the specification for a separate owning LibTorch dense compute representation, preserving the existing finite-MDP records and validation. Declare state--action flattening order, transition and expected-reward shapes, policy/value shapes, feasible-action and terminal masks, scalar type, device, memory requirements, and numerical tolerances. Use float64 for the finite-table reference computations; masked padding must never become invented dynamics for an infeasible pair.
- [ ] Accept or reuse the shared LibTorch tensor foundation before implementing the dense representation: a minimal C++20/CMake build, CPU tensor operations, CUDA operations and CPU/device agreement where available, and explicit device selection with CPU fallback. Record the shared setup once; automatic differentiation and neural optimisation remain prerequisites of the first neural consumer, not this table computation.
- [ ] Create the minimum `intelligence_foundations` and `synthetic_intelligence_engine` package skeletons when required; implement the finite table, validation, deterministic record-to-dense-tensor conversion, vectorised policy-induced transition calculation, and deterministic fixture in C++20/LibTorch. Build the numerical representation once per model/device configuration and reuse it; keep repeated kernels and intermediate values on the selected device rather than rebuilding tensors or transferring individual records inside each update.
- [x] Update the optional distribution lookup across the [specification](docs/05_optimisation_planning_decisions/finite_mdp_model.md), [implementation](ros_ws/src/intelligence_foundations/src/finite_mdp.cpp), [focused tests](ros_ws/src/intelligence_foundations/test/test_finite_mdp.cpp), and [companion](notes/part_05_optimisation_planning_decisions/finite_mdp_implementation.qmd). The learner reported passing tests; the focused rerun on 2026-09-07 passed all 31 finite-MDP cases with zero failures or errors. This is focused lookup/semantic-model evidence, not a new full-package closure run.
- [x] Complete the 2026-09-07 interactive companion-review batch for the implemented semantic model: optional/reference-wrapper access, construction and query connections, and the three-state analytic example with consistent mathematical labels and C++ indices. The example's C++ calls were compiled against the installed library and checked against independent expected values; the new diagram and example sections rendered separately to PDF, and temporary verification files were removed. The permanent artifact is [Finite-MDP Model Implementation](notes/part_05_optimisation_planning_decisions/finite_mdp_implementation.qmd); no complete-book revalidation or Cycle 1 closure is claimed.
- The [2026-09-05 overflow reproducer](docs/05_optimisation_planning_decisions/finite_mdp_model.md#unresolved-expected-reward-numerical-domain) is preserved in the owning specification; the extreme-reward policy remains unresolved. Final companion acceptance, registry updates, and cycle closure remain pending the numerical-domain decision and remaining Cycle 1 operations.
- [ ] Verify stochastic rows, invalid probabilities, non-finite values, incompatible dimensions, absorbing and terminal cases, entrywise record-to-tensor equivalence, flattening order, feasible-action masking, policy-induced transitions, and hand-calculated trajectory probabilities. Compare CPU and CUDA results within declared tolerances where CUDA is available; then write and review the retrospective companion and close Cycle 1.
...
- [ ] Implement finite-horizon backward induction and exact or converged policy evaluation in C++20/LibTorch using dense value vectors, policy tensors, and the verified transition/reward tensors for matrix--vector operations or linear solves. Vectorise independent states and batch independent problems while preserving horizon and iteration dependencies; use no automatic-differentiation graph for these non-neural calculations. Verify against hand-calculated values and direct joint-outcome reference calculations, including terminal handling, horizon/discount boundaries, stopping error, invalid policies, and CPU/CUDA agreement where available.
...
- [ ] Implement value iteration and policy iteration in C++20/LibTorch by reusing the dense transition/reward tensors, value vectors, and policy-evaluation operations. Use tensor operations for independent state--action Bellman calculations, masked action maximisation, and policy improvement. Preserve the specified update schedule and deterministic tie rule; do not replace sequential in-place updates with simultaneous updates without an explicit algorithm choice.
- [ ] Verify exact small-problem solutions, policy improvement, stable ties, contraction and stopping bounds where applicable, iteration limits, terminal cases, agreement between independent exact methods, and CPU/CUDA numerical agreement where available. Benchmark representative single and batched problems, including conversion, transfer, and synchronisation costs; retain CPU execution when GPU overhead outweighs the benefit.
...
- [ ] Specify tabular policy, value, exploration, seeding, episode, trace, and evaluation contracts against the `IF.4` exact oracle. Store Q-learning action values in a dense LibTorch state-by-action table with an explicit feasible-action mask, scalar type, and device; independent learner batches add a declared leading dimension. Q-learning consumes sampled transitions and does not require the environment's full dense transition tensor.
- [ ] Implement seeded bandit estimates, Monte Carlo prediction/control, TD(0), SARSA, and Q-learning in C++20 through the shared environment contract. Use the dense LibTorch Q table for action selection, masked next-state maxima, target arithmetic, and indexed updates on CPU or CUDA. Batch independent learners for GPU work while preserving each learner's sequential sample-update order: even updates to different entries can depend on earlier writes through their next-state targets. Do not silently turn online Q-learning into a simultaneous batch-update algorithm; parallel collection or evaluation requires independent seeded streams and a deterministic merge.
- [ ] Verify hand-calculated updates, terminal handling, action masking, exploration schedules, reproducible traces, and convergence toward the exact finite-MDP reference where assumptions permit. Compare tensor updates against an independent scalar reference, including repeated-entry and cross-entry dependencies, and compare each independent batch lane against its sequential reference. Check CPU/CUDA agreement on identical supplied samples where available and measure end-to-end throughput before choosing GPU execution for a workload.
...
- [ ] Reuse the accepted LibTorch tensor foundation from `IF.4` or its earlier owner. Accept the shared neural-update cycle, creating it here only if no earlier neural consumer owns it: automatic differentiation, optimiser step, and checkpoint round trip on the supported devices. Close and expose that cycle before continuing to DQN-specific work; do not create a second toolchain setup.
...
- [ ] Retain the architecture and claim boundaries in the owning capstone specification, and the data, baselines, ablations, metrics, compute, negative results, and exact foundation-core versions in its existing result manifests. Review this evidence, link it from a concise acceptance conclusion here, and close `IF.14`.
...
- [ ] Render and inspect the complete affected book, inspect the final diff and artifacts, record a concise track closure conclusion here with links to the existing acceptance evidence, and close `IF.15`.
